...
/// Describes whether to add a prefix `\x19Alaya Signed Message:\n`
...
impl AlayaSigner for JsonRpcSigner {
    /// The sign method calculates an Alaya specific signature with:
    /// checks if the server adds a prefix if not then adds
    /// return sign(keccak256("\x19Alaya Signed Message:\n" + len(message) + message))).
    async fn sign_message(&self, msg: &[u8]) -> Result<TxAtpSignature, SignerError> {
        let signature: PackedAtpSignature = {
            let msg = match &self.signer_type {
                ...
                None => {
                    return Err(SignerError::MissingAtpSigner);
                }
            };
        ...
        if is_signature_from_address(&signature, msg, self.address()?)? {
            Ok(TxAtpSignature::AlayaSignature(signature))
        } else {
...
impl JsonRpcSigner {
    pub async fn new(
        ...
        // AlayaSigner can support many different addresses,
    ...
    /// Specifies the Alaya address which sets the address for which all other requests will be processed.
    ...
    /// Server can either add the prefix `\x19Alaya Signed Message:\n` to the message and not add.
    ...
    pub async fn detect_signer_type(&mut self) -> Result<(), SignerError> {
        ...
        let msg_with_prefix = format!("\x19Alaya Signed Message:\n{}{}", msg.len(), msg);
...
mod tests {
    ...
    use zkdpos_types::{
        tx::{PackedAtpSignature, TxAtpSignature},
        Address,
    };

    use super::{is_signature_from_address, messages::JsonRpcRequest};
    use crate::{AlayaSigner, JsonRpcSigner, RawTransaction};
    ...
    async fn index(req: web::Json<JsonRpcRequest>, state: web::Data<State>) -> impl Responder {
        let resp = match req.method.as_str() {
            ...
            "eth_sign" => {
                ...
                let signature =
                    PackedAtpSignature::sign(state.key_pairs[0].secret(), &data_bytes).unwrap();
    ...
    async fn run_client() {
        ...
        if let TxAtpSignature::AlayaSignature(signature) =
