...
use zkdpos_types::{
    Account, AccountId, AccountUpdate, PubKeyHash, SignedZkDposTx, TokenId, ZkDposPriorityOp,
    ZkDposTx,
};
...
impl PlasmaTestBuilder {
    ...
    pub fn add_account(&mut self, state: AccountState) -> (AccountId, Account, PrivateKey) {
        ...
        let atp_sk = H256::random();
        let address = PackedAtpSignature::address_from_private_key(&atp_sk)
    ...
    pub fn test_tx_success(&mut self, tx: ZkDposTx, expected_updates: &BoundAccountUpdates) {
    ...
    pub fn test_tx_fail(&mut self, tx: ZkDposTx, expected_error_message: &str) {
        let error = self
            .state
            .execute_tx(tx)
    ...
    pub fn test_txs_batch_fail(&mut self, txs: &[SignedZkDposTx], expected_error_message: &str) {
