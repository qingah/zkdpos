...
use zkdpos_types::{Account, AccountUpdate, AccountUpdates, Deposit, DepositOp, ZkDposOp};
...
impl TxHandler<Deposit> for ZkDposState {
    type Op = DepositOp;
    ...
    fn apply_tx(&mut self, priority_op: Deposit) -> Result<OpSuccess, anyhow::Error> {
        ...
        let result = OpSuccess {
            fee,
            updates,
            executed_op: ZkDposOp::Deposit(Box::new(op)),
        };
