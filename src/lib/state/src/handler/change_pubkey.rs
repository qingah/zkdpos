...
use zkdpos_types::{
    operations::{ChangePubKeyOp, ZkDposOp},
    tx::ChangePubKey,
...
impl TxHandler<ChangePubKey> for ZkDposState {
    ...
    fn create_op(&self, tx: ChangePubKey) -> Result<Self::Op, anyhow::Error> {
        let (account_id, account) = self
            ...
            tx.is_atp_auth_data_valid(),
            "ChangePubKey Alaya auth data is incorrect"
        );
        ensure!(
            tx.verify_signature() == Some(tx.new_pk_hash),
            "ChangePubKey zkDpos signature is incorrect"
        );
        ensure!(
    ...
    fn apply_tx(&mut self, tx: ChangePubKey) -> Result<OpSuccess, anyhow::Error> {
        ...
        Ok(OpSuccess {
            fee,
            updates,
            executed_op: ZkDposOp::ChangePubKeyOffchain(Box::new(op)),
        })
    }
