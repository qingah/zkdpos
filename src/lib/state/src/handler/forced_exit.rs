...
use zkdpos_types::{AccountUpdate, AccountUpdates, ForcedExit, ForcedExitOp, PubKeyHash, ZkDposOp};
...
impl TxHandler<ForcedExit> for ZkDposState {
    ...
    fn apply_tx(&mut self, tx: ForcedExit) -> Result<OpSuccess, anyhow::Error> {
        ...
        Ok(OpSuccess {
            fee,
            updates,
            executed_op: ZkDposOp::ForcedExit(Box::new(op)),
        })
    }
