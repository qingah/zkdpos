...
use zkdpos_types::{AccountUpdate, AccountUpdates, FullExit, FullExitOp, ZkDposOp};
...
impl TxHandler<FullExit> for ZkDposState {
    type Op = FullExitOp;
    ...
    fn create_op(&self, priority_op: FullExit) -> Result<Self::Op, anyhow::Error> {
        ...
        let account_balance = self
            ...
            .filter(|account| account.address == priority_op.atp_address)
    ...
    fn apply_tx(&mut self, priority_op: FullExit) -> Result<OpSuccess, anyhow::Error> {
        ...
        let result = OpSuccess {
            fee,
            updates,
            executed_op: ZkDposOp::FullExit(Box::new(op)),
        };
