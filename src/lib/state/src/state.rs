...
pub struct OpSuccess {
    ...
    pub executed_op: ZkDposOp,
}

#[derive(Debug, Clone)]
pub struct ZkDposState {
...
impl ZkDposState {
    ...
    pub fn chunks_for_batch(&self, txs: &[SignedZkDposTx]) -> usize {
    ...
    pub fn chunks_for_tx(&self, franklin_tx: &ZkDposTx) -> usize {
        match franklin_tx {
            ZkDposTx::Transfer(tx) => {
                if self.get_account_by_address(&tx.to).is_some() {
                    TransferOp::CHUNKS
    ...
    pub fn execute_priority_op(&mut self, op: ZkDposPriorityOp) -> OpSuccess {
        match op {
            ZkDposPriorityOp::Deposit(op) => self
                .apply_tx(op)
                .expect("Priority operation execution failed"),
            ZkDposPriorityOp::FullExit(op) => self
                .apply_tx(op)
                .expect("Priority operation execution failed"),
        }
    }
    ...
    pub fn execute_txs_batch(&mut self, txs: &[SignedZkDposTx]) -> Vec<Result<OpSuccess, Error>> {
    ...
    pub fn execute_tx(&mut self, tx: ZkDposTx) -> Result<OpSuccess, Error> {
        match tx {
            ...
            ZkDposTx::ForcedExit(tx) => self.apply_tx(*tx),
        }
    }
    ...
    /// Converts the `ZkDposTx` object to a `ZkDposOp`, without applying it.
    pub fn zkdpos_tx_to_zkdpos_op(&self, tx: ZkDposTx) -> Result<ZkDposOp, anyhow::Error> {
        match tx {
            ...
            ZkDposTx::ForcedExit(tx) => self.create_op(*tx).map(Into::into),
        }
    }

    /// Converts the `PriorityOp` object to a `ZkDposOp`, without applying it.
    pub fn priority_op_to_zkdpos_op(&self, op: ZkDposPriorityOp) -> ZkDposOp {
        match op {
            ZkDposPriorityOp::Deposit(op) => self.create_op(op).unwrap().into(),
            ZkDposPriorityOp::FullExit(op) => self.create_op(op).unwrap().into(),
        }
    }
...
mod tests {
    ...
    fn execute_txs_batch_fail() {
        ...
        let signed_zk_sync_tx1 = SignedZkDposTx {
            tx: ZkDposTx::Withdraw(Box::new(withdraw1)),
            atp_sign_data: None,
        };
        let signed_zk_sync_tx2 = SignedZkDposTx {
            tx: ZkDposTx::Withdraw(Box::new(withdraw2)),
            atp_sign_data: None,
        };
    ...
    fn execute_txs_batch_fail_transfers() {
        ...
        let signed_zk_sync_tx1 = SignedZkDposTx {
            tx: ZkDposTx::Transfer(Box::new(transfer_1)),
            atp_sign_data: None,
        };
        let signed_zk_sync_tx2 = SignedZkDposTx {
            tx: ZkDposTx::Transfer(Box::new(transfer_2)),
            atp_sign_data: None,
        };
        tb.test_txs_batch_fail(
            &[signed_zk_sync_tx1, signed_zk_sync_tx2],
    ...
    fn delete_unexisting_account() {
        ...
        let mut state = ZkDposState::empty();
        let updates = vec![(
            AccountId(0),
    ...
    fn plasma_state_reversing_updates() {
        ...
        let initial_plasma_state = ZkDposState::from_acc_map(AccountMap::default(), BlockNumber(0));
