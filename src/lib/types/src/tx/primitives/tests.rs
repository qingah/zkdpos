...
// Local uses
use crate::{tx::*, AccountId, Nonce, Token, TokenId, Transfer, Withdraw, ZkDposTx};
...
#[test]
fn test_empty_batch() {
    assert!(AtpBatchSignData::new(Vec::new(), Vec::new()).is_err());
}

/// Checks the correctness of the message `AtpBatchSignData::new()` returns.
#[test]
fn test_batch_message() {
    let token = Token::new(TokenId(0), Default::default(), "ATP", 18);
    let transfer = get_transfer();
    let withdraw = get_withdraw();
    let change_pub_key = get_change_pub_key();
    let txs = vec![
        ZkDposTx::from(transfer.clone()),
        ZkDposTx::from(withdraw.clone()),
        ZkDposTx::from(change_pub_key.clone()),
    ];
    ...
    let txs = txs
        ...
        .collect::<Vec<_>>();
    // Shouldn't fail.
    let batch_sign_data = AtpBatchSignData::new(txs, Vec::new()).unwrap();
