...
/// `RemoveLiquidity` transaction performs a move of funds from one zkDpos account to another.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RemoveLiquidity {
    /// zkDpos network account ID of the transaction initiator.
    pub account_id: AccountId,
    /// Address of account to remove liquidity funds from.
    pub from: Address,
    /// Address of account to remove liquidity funds to.
    pub to: Address,
    /// Type of token for remove liquidity. Also represents the token in which fee will be paid.
    pub token: TokenId,
    /// Amount of funds to remove liquidity.
...
impl RemoveLiquidity {
    ...
    /// - remove liquidity recipient must not be `Adddress::zero()`.
