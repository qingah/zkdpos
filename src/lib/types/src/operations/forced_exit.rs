...
/// ForcedExit operation. For details, see the documentation of [`ZkDposOp`](./operations/enum.ZkDposOp.html).
...
impl ForcedExitOp {
    ...
    pub fn from_public_data(bytes: &[u8]) -> Result<Self, anyhow::Error> {
        ...
        let atp_address_end = atp_address_offset + ATP_ADDRESS_BIT_WIDTH / 8;
        ...
        .unwrap();
        let fee = unpack_fee_amount(&bytes[fee_offset..atp_address_offset])
            .ok_or_else(|| format_err!("Cant get fee from withdraw pubdata"))?;
        let target = Address::from_slice(&bytes[atp_address_offset..atp_address_end]);
