use zkdpos_crypto::primitives::GetBits;
...
use zkdpos_crypto::franklin_crypto::bellman::pairing::ff::PrimeField;

use super::Fr;
use super::{AccountId, AccountUpdates, Nonce, TokenId};
use zkdpos_basic_types::Address;
use zkdpos_crypto::circuit::account::{Balance, CircuitAccount};
use zkdpos_crypto::circuit::utils::atp_address_to_fr;
...
impl From<Account> for CircuitAccount<super::Engine> {
    fn from(acc: Account) -> Self {
        ...
        circuit_account.address = atp_address_to_fr(&acc.address);
        circuit_account
    }
}
