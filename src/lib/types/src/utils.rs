...
pub fn deserialize_atp_message<'de, D>(deserializer: D) -> Result<Vec<u8>, D::Error>
where
    D: Deserializer<'de>,
{
    struct StringOrVec;
...
/// Construct the first part of the message that should be signed by Alaya key.
...
pub fn alaya_sign_message_part(
    transaction: &str,
    token_symbol: &str,
    decimals: u8,
    amount: &BigUint,
