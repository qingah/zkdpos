...
use zkdpos_basic_types::{BlockNumber, U256};
use zkdpos_crypto::proof::EncodedAggregatedProof;
...
pub fn stored_block_info(block: &Block) -> Token {
    Token::Tuple(vec![
        ...
        Token::FixedBytes(block.get_atp_encoded_root().as_bytes().to_vec()),
        Token::FixedBytes(block.block_commitment.as_bytes().to_vec()),
    ])
}

impl BlocksCommitOperation {
    pub fn get_atp_tx_args(&self) -> Vec<Token> {
        ...
        let blocks_to_commit = self
            ...
            .map(|block| {
                ...
                Token::Tuple(vec![
                    Token::FixedBytes(block.get_atp_encoded_root().as_bytes().to_vec()),
                    Token::Bytes(block.get_atp_public_data()),
                    Token::Uint(U256::from(block.timestamp)),
...
impl BlocksProofOperation {
    pub fn get_atp_tx_args(&self) -> Vec<Token> {
        ...
        let proof = self.proof.get_atp_tx_args();
...
impl BlocksExecuteOperation {
    fn get_atp_tx_args_for_block(block: &Block) -> Token {
    ...
    pub fn get_atp_tx_args(&self) -> Vec<Token> {
        vec![Token::Array(
            self.blocks
                .iter()
                .map(BlocksExecuteOperation::get_atp_tx_args_for_block)
                .collect(),
        )]
    }
