//! zkDpos network block definition.

use super::PriorityOp;
use super::ZkDposOp;
use super::{AccountId, BlockNumber, Fr};
use crate::SignedZkDposTx;
use chrono::Utc;
use chrono::{DateTime, TimeZone};
use parity_crypto::digest::sha256;
use parity_crypto::Keccak256;
use serde::{Deserialize, Serialize};
use zkdpos_basic_types::{H256, U256};
use zkdpos_crypto::franklin_crypto::bellman::pairing::ff::{PrimeField, PrimeFieldRepr};
use zkdpos_crypto::params::{CHUNK_BIT_WIDTH, CHUNK_BYTES};
use zkdpos_crypto::serialization::FrSerde;

/// An intermediate state of the block in the zkDpos network.
...
pub struct ExecutedTx {
    pub signed_tx: SignedZkDposTx,
    pub success: bool,
    pub op: Option<ZkDposOp>,
...
impl ExecutedOperations {
    /// Returns the `ZkDposOp` object associated with the operation, if any.
    pub fn get_executed_op(&self) -> Option<&ZkDposOp> {
        match self {
    ...
    pub fn get_atp_public_data(&self) -> Vec<u8> {
        self.get_executed_op()
            .map(ZkDposOp::public_data)
            .unwrap_or_default()
    }

    /// Gets the witness required for the Alaya smart contract.
    /// Unlike public data, some operations may not have a witness.
    pub fn get_atp_witness_bytes(&self) -> Option<Vec<u8>> {
        self.get_executed_op()
            .map(|op| op.atp_witness().unwrap_or_else(Vec::new))
    }
...
pub struct Block {
    ...
    /// ID of the zkDpos account to which fees are collected.
    ...
    /// Gas limit to be set for the Commit Alaya transaction.
    pub commit_gas_limit: U256,
    /// Gas limit to be set for the Verify Alaya transaction.
...
impl Block {
    ...
    pub fn new_with_current_chunk_size(
        ...
        block.block_commitment = Block::get_commitment(
            ...
            block.get_atp_encoded_root(),
            block.timestamp,
            &block.get_onchain_op_commitment(),
            &block.get_atp_public_data(),
        );
        block
    }
    ...
    /// Returns the new state root hash encoded for the Alaya smart contract.
    pub fn get_atp_encoded_root(&self) -> H256 {
    ...
    /// Returns the public data for the Alaya Commit operation.
    pub fn get_atp_public_data(&self) -> Vec<u8> {
        let mut executed_tx_pub_data = self
            .block_transactions
            .iter()
            .filter_map(ExecutedOperations::get_executed_op)
            .flat_map(ZkDposOp::public_data)
            .collect::<Vec<_>>();
    ...
    /// Returns atp_witness data and data_size for each operation that has it.
    pub fn get_atp_witness_data(&self) -> (Vec<u8>, Vec<u64>) {
        let mut atp_witness = Vec::new();
        let mut used_bytes = Vec::new();

        for block_tx in &self.block_transactions {
            if let Some(franklin_op) = block_tx.get_executed_op() {
                if let Some(witness_bytes) = franklin_op.atp_witness() {
                    used_bytes.push(witness_bytes.len() as u64);
                    atp_witness.extend(witness_bytes.into_iter());
                }
            }
        }

        (atp_witness, used_bytes)
    }
    ...
    /// Returns the data about withdrawals required for the Alaya smart contract.
            ...
            if let Some(executed_op) = op.get_executed_op() {
                if executed_op.is_onchain_operation() {
                    onchain_ops.push(OnchainOperationsBlockInfo {
                        public_data_offset,
                        atp_witness: executed_op.atp_witness().unwrap_or_default(),
                    })
                }
