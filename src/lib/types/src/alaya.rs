//! Common primitives for the Alaya network interaction.
// Built-in deps
...
use zkdpos_basic_types::{Log, H256, U256};

/// Numerical identifier of the Alaya operation.
pub type AtpOpId = i64;

/// Type of the transactions sent to the Alaya network.
...
pub struct ATPOperation {
    ...
    /// Optional ZKDpos operation associated with Alaya operation.
    ...
    /// confirmed on the Alaya blockchain.
    pub confirmed: bool,
    /// Hash of the accepted Alaya transaction (if operation
    /// is confirmed).
...
impl PartialEq for ATPOperation {
    fn eq(&self, other: &Self) -> bool {
        // We assume that there will be no two different `ATPOperation`s with
...
/// Structure representing the result of the insertion of the Alaya
/// operation into the database.
/// Contains the assigned nonce and ID for the operation.
pub struct InsertedOperationResponse {
    /// Unique numeric identifier of the Alaya operation.
    pub id: i64,
    /// Nonce assigned for the Alaya operation. Meant to be used for all the
    /// transactions sent within one particular Alaya operation.
    pub nonce: U256,
}
