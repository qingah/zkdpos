...
use zkdpos_types::operations::RemoveLiquidityOp;
// Local deps
use crate::{
...
pub struct RemoveLiquidityWitness<E: RescueEngine> {
...
impl Witness for RemoveLiquidityWitness<Bn256> {
    type OperationType = RemoveLiquidityOp;
    ...
    fn apply_tx(tree: &mut CircuitAccountTree, remove_liquidity: &RemoveLiquidityOp) -> Self {
        ...
        Self::apply_data(tree, &remove_liquidity_data)
    }
    ...
    fn get_pubdata(&self) -> Vec<bool> {
        ...
        resize_grow_only(
            &mut pubdata_bits,
            RemoveLiquidityOp::CHUNKS * CHUNK_BIT_WIDTH,
            false,
        );
        pubdata_bits
    }
    ...
    fn get_offset_commitment_data(&self) -> Vec<bool> {
        vec![false; RemoveLiquidityOp::CHUNKS * 8]
    }
...
impl<E: RescueEngine> RemoveLiquidityWitness<E> {
...
impl RemoveLiquidityWitness<Bn256> {
    fn apply_data(tree: &mut CircuitAccountTree, remove_liquidity: &RemoveLiquidityData) -> Self {
        //preparing data and base witness
        let before_root = tree.root_hash();
        vlog::debug!("Initial root = {}", before_root);
        let (audit_path_from_before, audit_balance_path_from_before) =
            get_audits(tree, remove_liquidity.from_account_address, remove_liquidity.token);

        let (audit_path_to_before, audit_balance_path_to_before) =
            get_audits(tree, remove_liquidity.to_account_address, remove_liquidity.token);

        let capacity = tree.capacity();
        assert_eq!(capacity, 1 << account_tree_depth());
        let account_address_from_fe =
            Fr::from_str(&remove_liquidity.from_account_address.to_string()).unwrap();
        let account_address_to_fe = Fr::from_str(&remove_liquidity.to_account_address.to_string()).unwrap();
        let token_fe = Fr::from_str(&remove_liquidity.token.to_string()).unwrap();
        let amount_as_field_element = Fr::from_str(&remove_liquidity.amount.to_string()).unwrap();
        ...
        let fee_as_field_element = Fr::from_str(&remove_liquidity.fee.to_string()).unwrap();
        ...
        let valid_from = remove_liquidity.valid_from;
        let valid_until = remove_liquidity.valid_until;

        //applying first remove_liquidity part
        let (
            ...
            tree,
            remove_liquidity.from_account_address,
            remove_liquidity.token,
            |acc| {
        ...
        let (
            ...
            tree,
            remove_liquidity.to_account_address,
            remove_liquidity.token,
            |_| {},
