...
use zkdpos_types::operations::AddLiquidityOp;
// Local deps
use crate::{
...
pub struct AddLiquidityWitness<E: RescueEngine> {
...
impl Witness for AddLiquidityWitness<Bn256> {
    type OperationType = AddLiquidityOp;
    ...
    fn apply_tx(tree: &mut CircuitAccountTree, add_liquidity: &AddLiquidityOp) -> Self {
        ...
        Self::apply_data(tree, &add_liquidity_data)
    }
    ...
    fn get_pubdata(&self) -> Vec<bool> {
        ...
        resize_grow_only(
            &mut pubdata_bits,
            AddLiquidityOp::CHUNKS * CHUNK_BIT_WIDTH,
            false,
        );
        pubdata_bits
    }
    ...
    fn get_offset_commitment_data(&self) -> Vec<bool> {
        vec![false; AddLiquidityOp::CHUNKS * 8]
    }
...
impl<E: RescueEngine> AddLiquidityWitness<E> {
...
impl AddLiquidityWitness<Bn256> {
    fn apply_data(tree: &mut CircuitAccountTree, add_liquidity: &AddLiquidityData) -> Self {
        //preparing data and base witness
        let before_root = tree.root_hash();
        vlog::debug!("Initial root = {}", before_root);
        let (audit_path_from_before, audit_balance_path_from_before) =
            get_audits(tree, add_liquidity.from_account_address, add_liquidity.token);

        let (audit_path_to_before, audit_balance_path_to_before) =
            get_audits(tree, add_liquidity.to_account_address, add_liquidity.token);

        let capacity = tree.capacity();
        assert_eq!(capacity, 1 << account_tree_depth());
        let account_address_from_fe =
            Fr::from_str(&add_liquidity.from_account_address.to_string()).unwrap();
        let account_address_to_fe = Fr::from_str(&add_liquidity.to_account_address.to_string()).unwrap();
        let token_fe = Fr::from_str(&add_liquidity.token.to_string()).unwrap();
        let amount_as_field_element = Fr::from_str(&add_liquidity.amount.to_string()).unwrap();
        ...
        let fee_as_field_element = Fr::from_str(&add_liquidity.fee.to_string()).unwrap();
        ...
        let valid_from = add_liquidity.valid_from;
        let valid_until = add_liquidity.valid_until;

        //applying first add_liquidity part
        let (
            ...
            tree,
            add_liquidity.from_account_address,
            add_liquidity.token,
            |acc| {
        ...
        let (
            ...
            tree,
            add_liquidity.to_account_address,
            add_liquidity.token,
            |_| {},
