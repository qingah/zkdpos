...
impl<'a> WitnessBuilder<'a> {
    ...
    pub fn into_circuit_instance(self) -> ZkDposCircuit<'static, Engine> {
        ZkDposCircuit {
            rescue_params: &zkdpos_crypto::params::RESCUE_PARAMS,
            jubjub_params: &zkdpos_crypto::params::JUBJUB_PARAMS,
...
pub fn public_data_commitment<E: JubjubEngine>(
    ...
    // these two are BE encodings because an iterator is BE. This is also an Alaya standard behavior
...
impl SigDataInput {
    ...
    pub fn new(
        ...
        let r_bits: Vec<_> = zkdpos_crypto::primitives::BitConvert::from_be_bytes(&r_bytes)
            .iter()
            .map(|x| Some(*x))
            .collect();
        let s_bits: Vec<_> = zkdpos_crypto::primitives::BitConvert::from_be_bytes(&s_bytes)
            .iter()
            .map(|x| Some(*x))
            .collect();
        ...
        let sig_bits: Vec<bool> = zkdpos_crypto::primitives::BitConvert::from_be_bytes(&tx_bytes);
        ...
        let signer_pub_key_packed: Vec<_> =
            zkdpos_crypto::primitives::BitConvert::from_be_bytes(&signer_packed_key_bytes)
                .iter()
                .map(|x| Some(*x))
                .collect();
        Ok(SigDataInput {
...
pub fn build_block_witness<'a>(
    ...
    for op in ops {
        match op {
            ...
            ZkDposOp::FullExit(full_exit_op) => {
            ...
            ZkDposOp::ChangePubKeyOffchain(change_pkhash_op) => {
            ...
            ZkDposOp::ForcedExit(forced_exit) => {
            ...
            ZkDposOp::Noop(_) => {} // Noops are handled below
        }
    }
