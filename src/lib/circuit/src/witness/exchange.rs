...
use zkdpos_types::operations::ExchangeOp;
// Local deps
use crate::{
...
pub struct ExchangeWitness<E: RescueEngine> {
...
impl Witness for ExchangeWitness<Bn256> {
    type OperationType = ExchangeOp;
    ...
    fn apply_tx(tree: &mut CircuitAccountTree, exchange: &ExchangeOp) -> Self {
        ...
        Self::apply_data(tree, &exchange_data)
    }
    ...
    fn get_pubdata(&self) -> Vec<bool> {
        ...
        resize_grow_only(
            &mut pubdata_bits,
            ExchangeOp::CHUNKS * CHUNK_BIT_WIDTH,
            false,
        );
        pubdata_bits
    }
...
impl<E: RescueEngine> ExchangeWitness<E> {
...
impl ExchangeWitness<Bn256> {
    fn apply_data(tree: &mut CircuitAccountTree, exchange: &ExchangeData) -> Self {
        //preparing data and base witness
        let before_root = tree.root_hash();
        vlog::debug!("Initial root = {}", before_root);
        let (audit_path_from_before, audit_balance_path_from_before) =
            get_audits(tree, exchange.from_account_address, exchange.token);

        let (audit_path_to_before, audit_balance_path_to_before) =
            get_audits(tree, exchange.to_account_address, exchange.token);

        let capacity = tree.capacity();
        assert_eq!(capacity, 1 << account_tree_depth());
        let account_address_from_fe =
            Fr::from_str(&exchange.from_account_address.to_string()).unwrap();
        let account_address_to_fe = Fr::from_str(&exchange.to_account_address.to_string()).unwrap();
        let token_fe = Fr::from_str(&exchange.token.to_string()).unwrap();
        let amount_as_field_element = Fr::from_str(&exchange.amount.to_string()).unwrap();
        ...
        let fee_as_field_element = Fr::from_str(&exchange.fee.to_string()).unwrap();
        ...
        let valid_from = exchange.valid_from;
        let valid_until = exchange.valid_until;

        //applying first exchange part
        let (
