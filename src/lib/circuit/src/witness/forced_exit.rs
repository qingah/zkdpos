...
use zkdpos_crypto::{
    circuit::{
        ...
        utils::{append_be_fixed_width, atp_address_to_fr, le_bit_vector_into_field_element},
    },
    params::{
        account_tree_depth, ACCOUNT_ID_BIT_WIDTH, AMOUNT_EXPONENT_BIT_WIDTH,
        AMOUNT_MANTISSA_BIT_WIDTH, BALANCE_BIT_WIDTH, CHUNK_BIT_WIDTH, ATP_ADDRESS_BIT_WIDTH,
...
use zkdpos_types::operations::ForcedExitOp;
// Local deps
use crate::{
...
pub struct ForcedExitData {
    ...
    pub target_account_atp_address: Fr,
    pub valid_from: u64,
    pub valid_until: u64,
}
...
impl Witness for ForcedExitWitness<Bn256> {
    ...
    fn apply_tx(tree: &mut CircuitAccountTree, forced_exit: &ForcedExitOp) -> Self {
        ...
        let forced_exit_data = ForcedExitData {
            ...
            target_account_atp_address: atp_address_to_fr(&forced_exit.tx.target),
            valid_from,
            valid_until,
        };
    ...
    fn get_pubdata(&self) -> Vec<bool> {
        ...
        append_be_fixed_width(
            &mut pubdata_bits,
            &self.args.atp_address.unwrap(),
            ATP_ADDRESS_BIT_WIDTH,
        );
        resize_grow_only(
            &mut pubdata_bits,
    ...
    fn apply_data(tree: &mut CircuitAccountTree, forced_exit: &ForcedExitData) -> Self {
        ...
        ForcedExitWitness {
            ...
            args: OperationArguments {
                atp_address: Some(forced_exit.target_account_atp_address),
