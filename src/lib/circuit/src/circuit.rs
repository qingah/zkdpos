...
pub struct ZkDposCircuit<'a, E: RescueEngine + JubjubEngine> {
...
impl<'a, E: RescueEngine + JubjubEngine> std::clone::Clone for ZkDposCircuit<'a, E> {
    fn clone(&self) -> Self {
        Self {
...
impl<'a, E: RescueEngine + JubjubEngine> Circuit<E> for ZkDposCircuit<'a, E> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        {
            // Now it's time to pack the initial SHA256 hash due to Alaya BE encoding
...
impl<'a, E: RescueEngine + JubjubEngine> ZkDposCircuit<'a, E> {
    ...
    fn execute_op<CS: ConstraintSystem<E>>(
        ...
        // ensure op_data is equal to previous
        {
            ...
            is_op_data_correct_flags.push(CircuitElement::equals(
                cs.namespace(|| "is atp_address equal to previous"),
                &op_data.atp_address,
                &prev.op_data.atp_address,
            )?);
            is_op_data_correct_flags.push(CircuitElement::equals(
    ...
    fn withdraw<CS: ConstraintSystem<E>>(
        ...
        pubdata_bits.extend(op_data.atp_address.get_bits_be()); //ETH_ADDRESS=160
        ...
        *pubdata_holder = packed_pubdata;
        ...
        serialized_tx_bits.extend(op_data.atp_address.get_bits_be());
        ...
        serialized_tx_bits_old.extend(op_data.atp_address.get_bits_be());
    ...
    fn full_exit<CS: ConstraintSystem<E>>(
        ...
        let (is_pubdata_chunk_correct, pubdata_is_properly_copied) = {
            ...
            let pubdata_bits = {
                ...
                pub_data.extend(op_data.atp_address.get_bits_be()); //20
    ...
    fn deposit<CS: ConstraintSystem<E>>(
        ...
        pubdata_bits.extend(cur.token.get_bits_be()); //TOKEN_BIT_WIDTH=16
        pubdata_bits.extend(op_data.full_amount.get_bits_be()); //AMOUNT_PACKED=24
        pubdata_bits.extend(op_data.atp_address.get_bits_be()); //ATP_ADDRESS_BIT_WIDTH=160
        resize_grow_only(
            &mut pubdata_bits,
    ...
    fn change_pubkey_offchain<CS: ConstraintSystem<E>>(
        ...
        pubdata_bits.extend(op_data.atp_address.get_bits_be()); //ETH_KEY_BIT_WIDTH=160
        ...
        serialized_tx_bits_old.extend(cur.account_id.get_bits_be());
        serialized_tx_bits_old.extend(op_data.atp_address.get_bits_be());
        serialized_tx_bits_old.extend(op_data.new_pubkey_hash.get_bits_be());
        serialized_tx_bits_old.extend(cur.token.get_bits_be());
        serialized_tx_bits_old.extend(op_data.fee_packed.get_bits_be());
        serialized_tx_bits_old.extend(cur.account.nonce.get_bits_be());
        ...
        assert_eq!(
            serialized_tx_bits_old.len(),
            params::OLD_SIGNED_CHANGE_PUBKEY_BIT_WIDTH
        );
        ...
        *pubdata_holder = packed_pubdata;
        ...
        // Verify that zkDpos signature corresponds to the public key in pubdata.
        let is_signed_correctly = multi_and(
            cs.namespace(|| "is_signed_correctly"),
            &[
    ...
    fn transfer_to_new<CS: ConstraintSystem<E>>(
        ...
        pubdata_bits.extend(op_data.atp_address.get_bits_be()); //160
    ...
    fn forced_exit<CS: ConstraintSystem<E>>(
        ...
        pubdata_bits.extend(op_data.fee_packed.get_bits_be());
        pubdata_bits.extend(op_data.atp_address.get_bits_be());
        ...
        *pubdata_holder = packed_pubdata;
        ...
        // Check that `atp_address` corresponds to the rhs account Alaya address.
        let is_address_correct = CircuitElement::equals(
            cs.namespace(|| "is_address_correct"),
            &rhs.account.address,
            &op_data.atp_address,
...
fn generate_maxchunk_polynomial<E: JubjubEngine>() -> Vec<E::Fr> {
    use zkdpos_crypto::franklin_crypto::interpolation::interpolate;
