...
// Workspace deps
use zkdpos_crypto::params as franklin_constants;
// Local deps
use crate::{
...
pub struct AllocatedOperationData<E: Engine> {
    ...
    pub atp_address: CircuitElement<E>,
...
impl<E: RescueEngine> AllocatedOperationData<E> {
    pub fn empty_from_zero(zero_element: AllocatedNum<E>) -> Result<Self, SynthesisError> {
        let atp_address = CircuitElement::unsafe_empty_of_some_length(
            zero_element.clone(),
            franklin_constants::ATP_ADDRESS_BIT_WIDTH,
        );
    ...
    pub fn from_witness<CS: ConstraintSystem<E>>(
        ...
        let atp_address = CircuitElement::from_fe_with_known_length(
            cs.namespace(|| "atp_address"),
            || op.args.atp_address.grab(),
            franklin_constants::ATP_ADDRESS_BIT_WIDTH,
        )?;
