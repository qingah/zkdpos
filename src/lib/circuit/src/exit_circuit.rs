...
use zkdpos_types::{AccountId, TokenId};
// Local deps
use crate::{
...
#[derive(Clone)]
pub struct ZkDposExitCircuit<'a, E: RescueEngine> {
...
impl<'a, E: RescueEngine> Circuit<E> for ZkDposExitCircuit<'a, E> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let (state_root, _, _) = check_account_data(
            ...
            &branch,
            zkdpos_crypto::params::account_tree_depth(),
            self.params,
        )?;
...
pub fn create_exit_circuit_with_public_input(
    ...
    token_id: TokenId,
) -> ZkDposExitCircuit<'static, Engine> {
    ...
    ZkDposExitCircuit {
        params: &zkdpos_crypto::params::RESCUE_PARAMS,
...
mod test {
    use super::*;
    use num::BigUint;
    use zkdpos_crypto::circuit::account::CircuitAccount;
    use zkdpos_crypto::circuit::CircuitAccountTree;
    use zkdpos_crypto::franklin_crypto::circuit::test::TestConstraintSystem;
    use zkdpos_types::{Account, Nonce};

    #[test]
    #[ignore]
    fn test_zkdpos_exit_circuit_correct_proof() {
        ...
        let mut circuit_account_tree =
            CircuitAccountTree::new(zkdpos_crypto::params::account_tree_depth());
        circuit_account_tree.insert(*test_account_id, CircuitAccount::from(test_account));

        let zkdpos_exit_circuit = create_exit_circuit_with_public_input(
        ...
        zkdpos_exit_circuit.synthesize(&mut cs).unwrap();
